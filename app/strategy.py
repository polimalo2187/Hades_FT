...
def pullback_confirmation(df: pd.DataFrame, direction: str) -> bool:
    last = df.iloc[-1]
    if direction == "LONG":
        return last["close"] >= last["ema_fast"] * 0.998 and RSI_PULLBACK_MIN <= last["rsi"] <= RSI_PULLBACK_MAX
    else:
        return last["close"] <= last["ema_fast"] * 1.002 and RSI_PULLBACK_MIN <= last["rsi"] <= RSI_PULLBACK_MAX

def entry_confirmation(df: pd.DataFrame, direction: str) -> bool:
    last = df.iloc[-1]
    if direction == "LONG":
        return last["ema_fast"] > last["ema_slow"] * 1.0 and last["rsi"] > RSI_TREND_MIN - 2
    else:
        return last["ema_fast"] < last["ema_slow"] * 1.0 and last["rsi"] < RSI_TREND_MIN + 2
...
def mtf_strategy(
    ...
    # =====================
    # 15M → Pullback limpio y consistente con tendencia
    # =====================

    if pullback_confirmation(df_15m, direction):
        # Verificar consistencia con la tendencia de 15M
        if direction == "LONG" and df_15m.iloc[-1]["ema_fast"] < df_15m.iloc[-1]["ema_slow"]:
            return None
        elif direction == "SHORT" and df_15m.iloc[-1]["ema_fast"] > df_15m.iloc[-1]["ema_slow"]:
            return None
        score += 30
    ...
    # Distancia normalizada (ajustada para entrada más precisa, menos ruido)
    distance_pct = abs(last["close"] - last["ema_fast"]) / last["close"]
    entry_score = max(0, 30 - distance_pct * 1000)  # factor reducido para menos ruido
    ...
    # =====================
    # BONUS → Momentum fuerte alineado con tendencia
    # =====================
