...
def entry_confirmation(df: pd.DataFrame, direction: str) -> bool:
    last = df.iloc[-1]
    if direction == "LONG":
        return last["ema_fast"] > last["ema_slow"] * 0.999 and last["rsi"] > RSI_TREND_MIN - 2
    else:
        return last["ema_fast"] < last["ema_slow"] * 1.001 and last["rsi"] < RSI_TREND_MIN + 2
...
def mtf_strategy(
    ...
    # Distancia normalizada (ajustada para entrada más rápida)
    distance_pct = abs(last["close"] - last["ema_fast"]) / last["close"]
    entry_score = max(0, 30 - distance_pct * 2000)  # menor factor = entrada antes
