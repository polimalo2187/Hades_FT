...
from app.database import users_collection, signals_collection, user_signals_collection, get_client
...
async def scheduler_loop():
    ...
    try:
        ...
        if 'users' not in existing:
            ...
            logger.info("✅ Colección 'users' creada automáticamente")
        if 'signals' not in existing:
            db.create_collection('signals')
            logger.info("✅ Colección 'signals' creada automáticamente")
        if 'user_signals' not in existing:
            db.create_collection('user_signals')
            logger.info("✅ Colección 'user_signals' creada automáticamente")
    except Exception as e:
        logger.error(f"❌ Error creando colecciones automáticamente: {e}")
    # ==================================================
